...
When the user wants to look something up. Read the relevant glossary files and answer the question.
...
**Step 1: Find the glossary.** Search for existing glossary files using the discovery convention:
1. `glossary/glossary.config.yml` at project root
2. `docs/glossary/glossary.config.yml`
3. `.claude/glossary/glossary.config.yml`

If no glossary exists, switch to **Scaffolding** (see below).
...
**Step 5: Write the YAML.** Generate the term entry with the separator comment, append it to the correct glossary file, and regenerate `relationships.md` if any relationships were defined.

Show the user the generated YAML before writing it. Let them adjust.

### Maintain

When the user wants to check the health of the glossary. Load all glossary files and run checks:
...
Create the `glossary/` directory at the project root with:
...
If the project has any of these files, add a glossary pointer:

- **CLAUDE.md** — add a Glossary section pointing to `glossary/`
- **.cursorrules** — add a terminology note pointing to `glossary/`
- **README.md** — add a mention in the project structure section

Only touch files that already exist. Don't create new config files for tools the project doesn't use.

## How Other Skills Use the Glossary

The glossary is plain YAML in a known location. Any skill or tool can read it — no special API needed. The file format is the API.

When another skill (like gherkin) needs terminology:
1. Read `glossary/glossary.config.yml` to find context files
2. Read the relevant context file(s) based on scope
3. Use terms in its output (ui_label for user-facing text, code_name for technical text)

The glossary skill doesn't need to be invoked for this — other skills read the files directly.
...
| Tier | What | When |
|---|---|---|
| **Pointer** | 2 lines in CLAUDE.md | Every session (negligible) |
| **Scoped** | Relevant context file(s) + core | When this skill or another skill needs terminology |
| **Full** | All glossary files | Maintain mode only |

Use the `scope` field in `glossary.config.yml` to determine which context files are relevant. If working in `src/checkout/`, load `checkout.glossary.yml` + `core.glossary.yml`. If scope is ambiguous, ask: "Which context are you working in?"
